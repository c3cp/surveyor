  ...
  module Models
    module ResponseSetMethods
      ...
      def dependencies(question_ids = nil)
        #
        # SHWARE - NAIGLOBAL - workaround due to some sort of bug in Rails active record not using
        #  DependencyCondition.table_name on include conditions
        #
        ...
        deps = Dependency.all(:include => :dependency_conditions, :conditions => {DependencyCondition.table_name.to_sym => {:question_id => question_ids || responses.map(&:question_id)}})

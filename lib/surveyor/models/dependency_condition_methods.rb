  ...
  module Models
    module DependencyConditionMethods
      ...
      def to_hash(response_set)
        ...
        #
        # SHWARE - NAIGLOBAL - overwrite hardcoding of responses table name
        #
        #responses = question.blank? ? [] : response_set.responses.where("responses.answer_id in (?)", question.answer_ids).all
        responses = question.blank? ? [] : response_set.responses.where("#{Response.table_name}.answer_id in (?)", question.answer_ids).all
